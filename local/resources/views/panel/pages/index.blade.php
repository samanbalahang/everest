    ...
    <div class="table-responsive">
        <table id="zero_config" class="table table-inverse table-striped table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $i }}</td>
                    <td>@if ($item->image_url) <img src="{{ $item->image_url }}" width="60"> @else بدون تصویر @endif</td>
                    <td>{{ $item->title }}</td>
                    <td>{{ $item->slug }}</td>
                    <td>
                        @php
                    ...
                    <td>
                        {!! Form::open(['method' => 'DELETE', 'route' => ['admin.pages.destroy', $item->id], 'onsubmit' => "return confirm('آیا برای حذف مطمئن هستید؟');"]) !!}
                        <a href="{{ route('site.page', $item->slug) }}" class="btn btn-dark" target="_blank">
                            <i class="fa fa-eye"></i>
                        </a>
