...
<form action="{{Route("admin.area.store")}}" method="post">
	...
	<div class="my-3">
		<div class="d-flex">
			<div class="col-6">
				...
				<select name="state_id" id="state_id" onChange="iranwebsv(this.value);">
	...
	<div class="my-3 col-3">
		...
		<br>
		<div class="d-flex">
			<input type="text" id="areas" name="area" class="form-control">
			<input type="submit" class="btn btn-success" value="افزودن" id="add" required>
		</div>
	</div>
	<div class="my-3">
		...
		<div class="mydict">
			<div>
				...
				<label>
					<input type="radio" id="{{$area->area}}" name="area" value="{{$area->id}}" onclick="areaSelect(event)">
					<span>{{$area->area}}</span>
				</label>
...
<div class="md-3">
	<div class="d-flex">
		<div class="col-6">
			<form action="{{Route("admin.area.update",1)}}" method="post" class="d-flex gap-3" id="editForm">
				@csrf
				@method("put")
				<input type="hidden" name="state_id" value="" id="Editstate">
				<input type="hidden" name="city_id" value="" id="Editcity">
				<input type="text" id="area" name="area" class="form-control" style="display: none;">
				<input type="submit" class="btn btn-warning" value="ویرایش" id="edit" style="display:none">
			</form>
		</div>
		<!-- <div class="col-6">
			<form action="{{Route('admin.area.delete',1)}}" method="POST" id="deleteForm">
				@csrf
				@method('DELETE')
				<input type="submit" class="btn btn-danger" value="حذف" id="delete" style="display:none">
			</form>
		</div> -->
	</div>
</div>
...
<script>
	...
	function areaSelect(event) {
		...
		$("#area").val(event.target.id);
		var id = event.target.value;
		$("#area").show();
		$("#edit").show();
		$("#delete").show();
		$("#editForm").attr("action", "https://uni-everest.com/panel/area/" + id);
		$("#Editstate").val($("#state_id").val());
		$("#Editcity").val($("#city").val());
		$("#deleteForm").attr("action", "https://uni-everest.com/panel/area/" + id);
	}
