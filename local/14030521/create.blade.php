...
<main>
	...
	<div class="container">
		<div class="alert alert-warning" role="alert" id="alertrequired" style="display:none;">
			<strong>فرم ناقض است</strong> لطفاً تمام فیلد های اجباری را پر نمایید.
		...
		@endif
		<form action="{{route("site.signal.sabt")}}" method="post" id="form">
			@csrf
			<div class="row">
				<div class="col-4">
					<fieldset>
						<input type="hidden" name="karbar_id" id="karbar_id" value="{{$karbaran->id}}">
						<h2 class="w-100 text-center my-3 py-3">
							ثبت سیگنال
						</h2>
						<div class="my-3">
						...
						<div class="my-3">
							...
							<input type="tel" value="@if(isset($_GET["mobile"])){{$_GET["mobile"]}} @endif" @if(isset($_GET["mobile"])) {{'autofocus'}} @endif name="mobile" id="mobile" class="form-control" required onkeypress="allowNumbersOnly(event)" min="100000000">
						</div>
						<div class="my-3">
							<label for="mellicode" class="form-label">
								کد ملی
							</label>
							<input type="text" name="mellicode" id="mellicode" class="form-control" onkeypress="allowNumbersOnly(event)">
						</div>
						<div class="my-3">
							<label for="tozihat" class="form-label">
								توضیحات
							</label>
							<textarea type="text" name="tozihat" id="tozihat" class="form-control"></textarea>
						</div>
					</fieldset>
					<fieldset id="otherField">
						<div class="my-3">
							<div class="d-flex flex-wrap">
								<div class="col-6">
									...
									<input type="text" name="birthDay" id="birthDay" min="4" max="4" class="form-control" onkeypress="allowNumbersOnly(event)" onblur="calcAge(event)">
								</div>
								<div class="col-6">
									<label for="birthDay" class="form-label">
										سن
									</label>
									<input type="text" name="Age" id="Age" min="1" max="3" class="form-control" onkeypress="allowNumbersOnly(event)">
								</div>
							</div>
						</div>
						<div class="col-6">
						...
						<div class="my-3">
							...
							<div class="col-12">
								<label for="fieldLevel" class="form-label">
									مقطع تحصیلی
								</label>
								<select name="fieldLevel" id="fieldLevel" class="form-select form-control">
						...
						<div class="my-3">
							<div class="d-flex">
								<div class="col-6">
									...
									<select name="state" onChange="CityList(this.value);" class="form-control" id="state_id">
				...
				<div class="col-8">
					...
					<fieldset class="mt-3">
						<div class="my-3 border p-3">
							...
							<div class="mydict doreh">
								<div>
									@if($courses->count() > 0)
									@foreach($courses as $course)
									<label>
										<input type="checkbox" id="{{$course->title}}" name="course_id[]" value="{{$course->id}}">
						...
						<div class="my-3">
							...
							<textarea type="text" name="request" id="request" class="form-control"></textarea>
						</div>
						<hr>
						<div class="my-3 border">
							...
							<div class="mydict" id="thearea">
								<div>
									@if($areas->count() > 0)
									@foreach($areas as $area)
									<label>
										<input type="radio" id="{{$area->area}}" name="area" value="{{$area->id}}">
										<span>{{$area->area}}</span>
									</label>
									@endforeach
									@endif
						...
						<input type="submit" value="ثبت" class="my-3 w-100 btn btn-success">
					</fieldset>
				</div>
			</div>
...
<script>
				...
				if (isArray(response)) {
					if (response.length > 0) {
						response.forEach(element => {
							$("#thearea").append(`<label>
						<input type="radio" id="${element.area}" name="area" value="${element.id}" onclick="areaSelect(event)">
						<span>${element.area}</span>
						</label>`);
						});
					} else {
						$("#thearea").html("<div></div>");
					}
				}
			}
		});
	...
	$("#form").on("submit", function() {
		...
		if ($(this).find("input[required]").val() == "") {
			$("#alertrequired").toggle();
			return false;
		}
		if ($(".mydict").find("input[type=checkbox]:checked").length > 0) {
			return true;
		} else {
			// console.log("ELSE");
			$("#alertrequired").toggle();
			$(".doreh").addClass("bg-warning");
			return false;
		}
		return false;
	})
	...
	function calcAge(event) {
		...
		if (event.target.value != "") {
			let birthDay = parseInt(event.target.value);
			let thisyear = new Date().toLocaleDateString('fa-IR');
			thisyear = thisyear.toString();
			thisyear = thisyear.split("/")[0];
			thisyear = parseInt(fixNumbers(thisyear));
	...
	function iranwebsv(state) {
		with(document.getElementById('city')) {
			...
			if (state == 31) {
				...
				options[7] = new Option('كرج', '31', true, true);
	...
	$("#mobile").on("blur", function() {
		...
		$.ajax({
			...
			success: function(result) {
				if (result != "") {
					let olduser = document.createElement("input");
					olduser.type = "hidden";
					olduser.id = "olduser";
					olduser.name = "olduser";
					olduser.value = result.id;
					$("#form").append($(olduser));
					$("#fname").val(result.fname);
					$("#lname").val(result.lname);
					$("#mellicode").val(result.mellicode);
					$("#tozihat").val(result.tozihat);
					...
					Swal.fire({
						text: "کاربر قبلاً ثبت شده داده های قبلی را مشاهده خواهید کرد.",	
					});
				}
			}
		});
	...
	function getoldsignal(id) {
		...
		let div;
		$.ajax({
			url: "{{route('site.signal.oldsignals') }}",
			data: data,
			type: 'post',
			success: function(result) {
				if (result != "") {
					...
					$("#oldsignal").addClass("mt-5 pt-5 border");
					$("#oldsignal").append("<h4 class='px-3'>سیگنال های قبلی کاربر</h4>");
					if (typeof result == "string") {
						result = JSON.parse(result);
						for (let index = 0; index < result.length; index++) {
							const element = result[index];
							// console.log("element.id:" + element.id);
							var course = "{{App\Course::find(" + element.id + ")}}";
							// console.log(course);
							$("#oldsignal").append(`
							<div>
								<p> رشته: <strong>${element.course}</strong></p>
								<p> درخواست ها :${element.request? element.request: ""}</p>
							</div>`);
							
							var course_id = parseInt(element.course_id);
							console.log(element.course);
							console.log(element);
							console.log(element.course_id);
							console.log(course_id);
							console.log($(`input[name="course_id[]"]`));
							console.log($(`input[name="course_id[]"][value="${course_id}"]`));
							$(`input[name="course_id[]"][value="${course_id}"]`).prop( "checked", true );
						}
					} else {
